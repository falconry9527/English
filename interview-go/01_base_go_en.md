...
```
一. 定义和初始化:
 A slice in Go is a reference type 
 that encapsulates 
 pointer : Pointer to a array
 len :  Current length
 cap : Current capacity
 
 slice 是一个 封装类型，封装了 指针（指向底层数组）,len(当前长度),cap (当前容量)
type slice struct {
...
Goroutine 的切换具体是  GPM调度模型：开始讲 GPM调度模型

Goroutine 是 go的并发机制-csp并发模型的核心，开始讲 csp并发模型
...
每个 M 绑定一个 P。
M 从 P 获取 G 并执行。
a. G 阻塞 : G 会被 P 放入等待队列（wait queue），等待唤醒
