...
触发条件
s = append(s, x)
当添加元素的时候，如果数组容量不够，就会 新创建一个数组，把旧数组的数据copy 到新数组，切换指针到新的数组
...
new： 适用于值类型（如 int、float、struct、数组等）；
也能用于引用类型（slice、map、channel） ; 
返回该类型的指针，也就是说只定义类型，不开辟内存空间，不创建底层数组（slice）。

make：适用于引用类型（slice、map、channel） ; 
返回的是它们本身，也就是不光定义类型，也开辟内存空间，创建底层数组（slice）。

为什么 slice 推荐 make：slice 是个 {ptr, len, cap} 的结构体，new 只把这个结构体清零（ptr=nil），底层数组还没创建；make 会把底层数组也分配好，返回一个真正可用的 slice。
...
核心理念：通过通信来共享内存,实现并发；而不是通过共享新 内存实现tong
...
协程它由编程语言层面实现GPM 调度模型，上下文切换(协程切换)不需要经过内核态 （操作系统）
...
P（Processor）: 资源调度器，负责分配 G 到 M 上运行。
