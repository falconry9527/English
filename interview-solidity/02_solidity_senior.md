...
中间计算结果：使用 memory,calldata  存储 ，以降低存储的gas 消耗。
tick 模型：使用 位移运算，替代 乘以2 和 除以2 ；常量预计算，把 （10001/10000 ）的 1，2，4，8，26，64 次方，等 常用常量预先计算，避免多次计算
只更新必要的字段： 比如在自建预言机的时候，如果两次价格不变，就只更新时间，不更新价格，减少数据存储的gas 消耗

assembly :
assembly 使是 EVM 的汇编语言，允许：
直接操作内存
直接读写 storage
手动控制循环和跳转
调用底层 opcode

assembly 可以去掉很多 Solidity 自动生成的开销：
边界检查
溢出检查
多余的内存操作
ABI 编码开销
...
## Hardhat 和 Foundry
```
都是智能合约开发框架，覆盖：编译、测试、本地链（支持主网分叉）、部署。

Hardhat：用 JS/TS，生态成熟，调试强（Solidity 里直接 console.log），插件多，前端集成方便（ethers.js）。

Foundry：用 Rust，用 Solidity 写测试，速度极快。
        招牌是 Fuzz 测试（带参数的测试函数自动喂随机值找边界 bug）和
        Cheatcodes（vm.prank 伪装地址、vm.warp 改时间等）。安全审计圈首选。
...
EIP-7702: 用户签名个授权，发给paymaster,由 paymaster 发起交易，并支付gas
用户:    出离线签名 —— 不花 ETH
           ① 委托签名(7702 authorization):仅首次,持久委托
           ② 意图签名(amount/recipient/nonce/deadline):每次
paymaster:出 gas(当 type-4 交易发送方)—— 花自己的 ETH,碰不到用户的钱
delegate: 代码跑在用户地址上,msg.sender=用户 —— 动的是用户资产
