...
```
代理合约 : 保存状态变量，接收用户调用，通过 delegatecall 转发逻辑。
实现合约:  由于使用的是 delegatecall ，所以，实现合约修改的是调用者（代理合约）的状态变量。

代理合约和逻辑合约, slot 分配顺序可能相同，比如都是从0开始，数据存储在一个 storage，容易 数据覆盖 或 存储冲突。

解决方法: EIP-1967
给代理合约的 变量分配 固定、特殊的 slot ，通过哈希计算得到，保证不会与逻辑合约的业务变量 slot 冲突：
