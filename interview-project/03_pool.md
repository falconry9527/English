...
----- 利息指数
核心：利息指数(每单位资产累计利息) = 借贷产生的利息 ÷ 池子总资产数量
场景: 池子在一段时间内产生利息 Interest（池子总资产数量 × 利率 × 时间）

1. 池子维护了 一个池子的利息指数，每次更新 :
池子最新利息指数 totalInterest = 旧的利息指数 totalInterestOld + Interest ÷ 池子总资产数量

利息指数, 避免了池子流动性（资产）变化的干扰

2. 流动性提供者（出借方）维护了一个用户的利息指数，每次更新 :
a.出借方新增利息 = （池子最新利息指数 totalInterest - 用户上次记录的利息指数 userInterest） × 出借方存入的资产数量
b.更新用户的利息指数记录：userInterest = totalInterest

更新策略：
1.定时更新
2.触发更新 : 查看账户信息 或者 移除流动性(赎回资产)的时候
...
怎么防护:
1. 聚合多个数据来源，取最新提交价格的中位数(预言机: charlink 多节点最新提交价格的中位数 ,pyth,redtone)
2. 时间加权价格（TWAP）+ 延迟确认机制  :  增大时间窗口（30分钟），减少单次报价冲击影响。
...
```
1.  为什么使用白名单机制
a. 需要经过 KYC（身份认证） / AML（反洗钱认证）,增加信任度
b. 进行后续的权限管理，开放不同的借贷池权限给用户

2. 利息 和质押率
出借利息 :  年化: 3-50% 
借款利息 :  年化: 5-80% 
质押率 ：  300% - 1000% （借贷率： 10%-50% ）

3. 为什么不直接卖出，而是质押借贷
a. 避免大量抛售，压低价格
b. 如果长期看好代币，可以在保留代币的同时，获取资金，进行其他领域的投资

4. 清算 获得的是质押的资产，不是卖出的usdt
```
